...
local function getPowerPerClick(player: Player): number
	local data = (_G :: any).VillainsData
	if not data then return Config.Training.ClickBase end
	local d = data.Get(player)
	local villainName = d.EquippedVillain
	local v = Config.GetVillainByName(villainName)
	local base = 1
	if v and v.PowerPerClick then base = v.PowerPerClick
	elseif v and v.IsBestMultiplier then
		...
		for _, owned in d.OwnedVillains do
			...
			if ov and ov.PowerPerClick and ov.PowerPerClick > best then best = ov.PowerPerClick end
		end
		base = best * 2
	end
	...
	if hasPass(player, "2x Infamy") then total *= 2 end
...
trainEv.OnServerEvent:Connect(function(player: Player)
	...
	if now - last < Config.Training.ClickCooldown then return end
	lastClick[player] = now

	local amount = getPowerPerClick(player)
	local data = (_G :: any).VillainsData
	if data then data.AddInfamy(player, amount) end
end)
...
pad.Touched:Connect(function(hit)
	...
	if plr then onPad[plr] = true end
end)
pad.TouchEnded:Connect(function(hit)
	local plr = Players:GetPlayerFromCharacter(hit.Parent)
	if plr then onPad[plr] = false end
end)
	...
	while true do
		...
		for plr, active in onPad do
			if active then
				...
				local data = (_G :: any).VillainsData
				if data then data.AddInfamy(plr, amount) end
			end
		end
...
(_G :: any).GetPowerPerClick = getPowerPerClick
